...
// split the aera of the memory by address and assert the sel_flag signal 
   assign sel0 = (addr[2:1] == 2'd0);
   assign sel1 = (addr[2:1] == 2'd1);
   assign sel2 = (addr[2:1] == 2'd2);
   assign sel3 = (addr[2:1] == 2'd3);
//busy is for working, the memread and memwrite need more than 1 cycle, when the bank could be operate, en=1, or it is busy, en=0
//once it be busy and need to be aperate, the memory should be stall until everything done

//En get 4 bits for the banks are working or not
   wire [3:0]          en;
   ...
   assign busy = bsy0 | bsy1 | bsy2;// when the en get 4'd1, it will set busy automatically because all these bankes are working
